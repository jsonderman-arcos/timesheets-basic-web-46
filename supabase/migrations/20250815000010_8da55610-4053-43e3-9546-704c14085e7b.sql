-- Clear existing timesheet data and add comprehensive fake data
DELETE FROM public.timesheets;

-- Add some sample exceptions and GPS data for a few timesheets later
-- Let's focus on creating the timesheet data first
...
timesheet_data AS (
  SELECT 
    ...
    CASE 
      -- 2 crews (crew numbers 24, 25) never submit (0% compliance) - UtilityMax teams
      WHEN c.crew_num IN (24, 25) THEN false
      -- 5 crews (crew numbers 19-23) submit 80% of the time - ElectriCo teams
...
SELECT 
  ...
  td.work_date,
  td.start_time,
  (td.start_time + (td.work_hours || ' hours')::interval + '30 minutes'::interval)::time as end_time,
  td.work_hours,
  CASE 
    WHEN td.name LIKE '%PowerCorp%' THEN 
      ...
      END
    WHEN td.name LIKE '%GasWorks%' THEN
      CASE (ABS(EXTRACT(epoch FROM td.work_date)::integer + td.crew_num) % 4)
      ...
      END
    WHEN td.name LIKE '%AquaTech%' THEN
      CASE (ABS(EXTRACT(epoch FROM td.work_date)::integer + td.crew_num) % 4)
      ...
      END
    WHEN td.name LIKE '%ElectriCo%' THEN
      CASE (ABS(EXTRACT(epoch FROM td.work_date)::integer + td.crew_num) % 4)
  ...
  (CASE 
    ...
    ELSE 'rejected'
  END)::timesheet_status as status,
