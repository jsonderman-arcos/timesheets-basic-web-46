...
(
  ...
  NOW() - INTERVAL '2 days',
  'Accepted - Valid sick leave with proper documentation',
  NOW() - INTERVAL '5 days'
),
(
  gen_random_uuid(),
  ...
  (SELECT id FROM users WHERE role = 'utility' LIMIT 1),
  NOW() - INTERVAL '1 day',
  'Accepted - Safety critical work, overtime justified',
  NOW() - INTERVAL '3 days'
),
(
  gen_random_uuid(),
  ...
  'Accepted - Documented traffic incident, unavoidable delay',
  NOW() - INTERVAL '1 day'
),

-- Pending exceptions (submitted status)
(
  gen_random_uuid(),
...
(
  ...
  (SELECT id FROM users WHERE role = 'utility' LIMIT 1),
  NOW() - INTERVAL '4 days',
  'Accepted - Safety evacuation, proper protocol followed',
  NOW() - INTERVAL '6 days'
),
...
(
  ...
  'Rejected - Should have arranged backup transportation sooner',
  NOW() - INTERVAL '2 days'
),
